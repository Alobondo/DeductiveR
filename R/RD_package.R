#' Apply the rational deductive method to a monthly series of flow or
...
RD <- function(data){
  ...
  # and a record with at least 10 full years.
  ...
  if(complete_y < 10){stop("There is less than 10 complete years")}
  ...
  data_wide <- cast(data, year ~ month)
  ...
  # Auxiliary vectors to generate fill values with RD method
  ...
  RD <- ((Rep_aux_2 * Rep_aux_3) / Rep_aux)*Rep_aux_4

  # Final element with existing and filled data
  RD_final <- data_wide %>% select(-year, -Suma)
  RD_final[is.na(RD_final)] <- 0

  RD <- t(matrix(RD,nrow = 12))
  RD_final <- RD_final + RD

  RD_final <- cbind(data_wide$year, RD_final)
  colnames(RD_final) <- c("year",month.abb[seq(1,12)]) # Column names
